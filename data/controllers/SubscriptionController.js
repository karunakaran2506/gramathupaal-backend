...
const CreateSubscriptionpack = async function (req, res) {
  const promise = new Promise(async function (resolve, reject) {
    let validParams =
      req.body.name && req.body.price && req.body.validity && req.body.product;
      ...
      try {
        ...
        let subscriptionpack = await Subscriptionpack.create({
          ...
          resolve({
            status: 200,
            success: true,
            message: "Subscriptionpack created successfully",
          });
        });
      } catch (error) {
...
const EditSubscriptionpack = async function (req, res) {
  const promise = new Promise(async function (resolve, reject) {
    let validParams =
      ...
      req.body.price &&
      req.body.validity &&
      req.body.subscriptionpack;

    if (validParams) {
      try {
        let subscriptionpack = await Subscriptionpack.updateOne(
          { _id: req.body.subscriptionpack },
          {
            $set: {
              name: req.body.name,
              price: req.body.price,
          ...
          resolve({
            status: 200,
            success: true,
            message: "Subscriptionpack edited successfully",
          });
        });
      } catch (error) {
        reject({ status: 200, success: false, message: error.message });
      ...
      let product = await Subscriptionpack.find({})
        ...
        .then((data) => {
          resolve({
            status: 200,
            success: true,
            message: "Subscription pack list",
  ...
  const promise = new Promise(async function (resolve, reject) {
    ...
    if (validParams) {
      try {
        ...
        const result = await Subscriptionorders.findOne({
          customer: req.body.customer,
          subscriptionpack: req.body.subscriptionpack,
        });
        ...
        if (result) {
          const updatedValidity =
            Number(result.validity) + Number(req.body.validity);
          await Subscriptionorders.findByIdAndUpdate(
            result._id,
...
const EditSubscriptionorder = async function (req, res) {
  const promise = new Promise(async function (resolve, reject) {
    ...
    if (validParams) {
      try {
        let subscriptionpackorder = await Subscriptionorders.updateOne(
          { _id: req.body.subscriptionorder },
          {
            $set: {
              deliveryman: req.body.deliveryman,
              customeraddress: req.body.customeraddress,
      ...
      try {
        let subscriptionpackorder = await Subscriptionorders.updateOne(
          ...
          resolve({
            status: 200,
            success: true,
            message: "Subscription order deactivated successfully",
          });
        });
      } catch (error) {
        reject({ status: 200, success: false, message: error.message });
...
const ListSubscriptionorderbyCustomer = async function (req, res) {
  const promise = new Promise(async function (resolve, reject) {
    try {
      let product = await Subscriptionorders.find({
        customer: req.body.customer,
      })
        ...
        .then((data) => {
          resolve({
            status: 200,
            success: true,
            message: "Subscription order list",
            order: data,
          });
        });
    } catch (error) {
...
const ListActiveSubscriptionorderbyDeliveryMan = async function (req, res) {
  const promise = new Promise(async function (resolve, reject) {
    try {
      let product = await Subscriptionorders.find({
        store: req.body.store,
      ...
      try {
        ...
        let subscriptionpack = await Deliveryavailablity.create({
          ...
          resolve({
            status: 200,
            success: true,
            message: "Delivery availablity created successfully",
          });
        });
      } catch (error) {
        reject({ status: 200, success: false, message: error.message });
        ...
        .then(async (data) => {
          ...
          for (let i = 0; i < data.length; i++) {
            ...
            const deliveryEntry = await Deliveryentry.find({
              subscriptionpackorder: order._id,
              entrydate: { $gte: start, $lt: end },
            });
            const isPending = deliveryEntry.length ? false : true;
            const isDelivered = deliveryEntry.length
              ? deliveryEntry[0].isdelivered
              : false;

            const availablity = await Deliveryavailablity.find({
              subscriptionpackorder: order._id,
              entrydate: { $gte: start, $lt: end },
            });
            const isAvailable = availablity.length
              ? availablity[0].availablity
              : true;
...
const ActiveSubscriptionsByDeliveryman = async function (req, res) {
  const promise = new Promise(async function (resolve, reject) {
    try {

      await Subscriptionorders.find({ deliveryman: req.body.deliveryman, validity: { $gt: 0 } })
        .select("_id subscriptionpack customer customeraddress validity")
        .populate({
          path: "subscriptionpack",
          select: "name",
        })
  ...
  const promise = new Promise(async function (resolve, reject) {
    ...
    if (validParams) {
      try {
        ...
        const result = await Subscriptionorders.findById(
          req.body.subscriptionpackorder
        );
        ...
        const createDeliveryEntry = async () => {
          await Deliveryentry.create({
            ...
            resolve({
              status: 200,
              success: true,
              message: "Deliver entry created successfully",
            });
          });
        };
...
const ListSubscriptionHistorybyStore = async function (req, res) {
  const promise = new Promise(async function (resolve, reject) {
    try {
      ...
      let product = await Subscriptionhistory.find({
        store: req.body.store,
        ...
        .then((data) => {
          resolve({
            status: 200,
            success: true,
            message: "Subscription history list",
            history: data,
          });
        });
    } catch (error) {
...
const ListDeliveryEntriesbyDate = async function (req, res) {
  const promise = new Promise(async function (resolve, reject) {
    try {
      let newdate = req.body.date;
      const start = new Date(new Date(newdate).setHours(0, 0, 0, 0));
      ...
      if(req.body.deliveryman !== 'all'){
        condition = {
          ...condition,
          deliveredby: req.body.deliveryman,
        };
      };

      await Deliveryentry.find(condition)
        .populate({
          path: "subscriptionpackorder",
          select: "subscriptionpack customer",
          populate: {
            path: "subscriptionpack customer",
            select: "name",
          },
        })
...
const ViewDeliveryHistory = async function (req, res) {
  const promise = new Promise(async function (resolve, reject) {
    try {
      await Deliveryentry.find({
        subscriptionpackorder: req.body.subscriptionpackorder,
      })
        .select("isdelivered entrydate notes")
...
const ListActiveSubscriptionorderbyCustomer = async function (req, res) {
  const promise = new Promise(async function (resolve, reject) {
    try {
      let product = await Subscriptionorders.find({
        customer: req.body.customer,
        ...
        .select("subscriptionpack deliveryman validity")
        .populate("subscriptionpack", "name validity price")
        .populate("deliveryman", "name phone")
        .then((data) => {
          resolve({
...
module.exports = {
  ...
  ListActiveSubscriptionorderbyDeliveryMan,
  ...
  ListDeliveryEntries,
  ViewDeliveryHistory,
  ListDeliveryEntriesbyDate,
  ListActiveSubscriptionorderbyCustomer,
  ActiveSubscriptionsByDeliveryman,
};
