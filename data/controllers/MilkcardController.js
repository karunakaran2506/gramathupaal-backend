      ...
      try {
        ...
        let milkcard = await Milkcard.create({
          ...
          resolve({
            status: 200,
            success: true,
            message: "Milkcard created successfully",
          });
        });
      } catch (error) {
...
const EditMilkcard = async function (req, res) {
  const promise = new Promise(async function (resolve, reject) {
    let validParams = req.body.id;

    if (validParams) {
      try {
        let milkcard = await Milkcard.updateOne(
          { _id: req.body.id },
          {
            $set: {
              name: req.body.name,
          ...
          resolve({
            status: 200,
            success: true,
            message: "Milkcard edited successfully",
          });
        });
      } catch (error) {
        reject({ status: 200, success: false, message: error.message });
...
const DeleteMilkcard = async function (req, res) {
  const promise = new Promise(async function (resolve, reject) {
    let validParams = req.body.id;
...
const CreateMilkcardEntry = async function (req, res) {
  const promise = new Promise(async function (resolve, reject) {
    let validParams =
      req.body.customer && req.body.validity && req.body.milkcard;
    ...
    if (validParams) {
      try {
        ...
        const result = await Milkcardentry.findOne({
          customer,
          milkcard: req.body.milkcard,
        });
        ...
        if (result) {
          ...
          await Milkcardentry.findByIdAndUpdate(
            ...
            resolve({
              status: 200,
              success: true,
              message: "Milkcard entry created successfully",
            });
          });
        } else {
...
const ListMilkcardEntrybyCustomer = async function (req, res) {
  const promise = new Promise(async function (resolve, reject) {
    try {
      let product = await Milkcardentry.find({ customer: req.body.customer })
        .populate("milkcard")
        .then((data) => {
          resolve({
            status: 200,
            success: true,
            message: "Milkcard entry list",
            entry: data,
          });
        });
    } catch (error) {
...
const CustomerActiveMilkcard = async function (req, res) {
  const promise = new Promise(async function (resolve, reject) {
    try {
      let product = await Milkcardentry.find({ customer: req.body.customer, validity : { $ne: 0} })
        .select("milkcard validity _id")
        .populate({
          path: "milkcard",
...
const ListMilkcardHistorybyStore = async function (req, res) {
  const promise = new Promise(async function (resolve, reject) {
    try {
      ...
      let product = await Milkcardhistory.find({
        store: req.body.store,
